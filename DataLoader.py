...
def load_images (folder, patch_size, random=True, n_patches=3, stride=32, cut_size=None, preprocess_limit = 100, resize=None):
    images = []
    for file in os.listdir(folder):
        if file.endswith(".bmp") or file.endswith(".tif") or file.endswith(".png"):
            _ , img = load_patches_from_file(os.path.join(folder, file), patch_size, random, n_patches, stride, cut_size, preprocess_limit, resize)
            #plt.imshow(img)
            #plt.show()
            images.append(img)
    return images
